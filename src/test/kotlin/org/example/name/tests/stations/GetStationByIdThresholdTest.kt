...
class GetStationByIdThresholdTest : BaseTest() {
    @Test(dataProvider = "JsonTestData")
    @TestDataKeys("stationId", "testId", "ghej")
    fun getStationByIdThresholdTest(stationId: String, testId: String, ghej: String) {
        val hej = testId
        val yo = stationId
        val tsss = ghej.toBoolean()
        val stationsApiService = StationsApiService()
        stationsApiService.getStationByIdMeetsPerformanceThreshold(stationId)
    }
}
